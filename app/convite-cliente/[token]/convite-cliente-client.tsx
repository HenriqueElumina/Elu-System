...
import {
  ...
  type CompleteClientUserSignupInput,
...
export function ConviteClienteClient({ token }: { token: string }) {
  ...
  useEffect(() => {
    ...
    async function check() {
      ...
      const { data, error: rpcError } = await supabase
        .rpc("get_client_user_invite", { p_token: token })
  ...
  async function handleSubmit(event: React.FormEvent) {
    ...
    const parsed = completeClientUserSignupSchema.safeParse({
      fullName,
      password,
    } satisfies CompleteClientUserSignupInput);
    if (!parsed.success) {
    ...
    const { error: submitError } = await supabase.rpc("submit_client_user_invite", {
      p_token: token,
